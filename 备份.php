<?php   
    ...
    $week=""; //值班星期
    $duty_time=""; //值班时间
    $holidy=array(5,6,7,8,9); //假期时间
    $user_index=0; //值班人员顺序
    $leader_index=0; //值班领导顺序
    ...
    echo date('t', strtotime($Y.'-'.$M));
    $duty_day=date('t', strtotime($Y.'-'.$M));
    //程序开始
    echo date('W',strtotime('2018-5-1'));
    ...
    $db->setDutyIndex(($user_index-1),($leader_index-1));//记录下月次数
